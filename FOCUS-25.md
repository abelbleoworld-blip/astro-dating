...
2. Затем **JOURNEY.md** — логические цепочки откуда взялся каждый артефакт
3. Затем `docs/lectures/INDEX.md` — карта 8 лекций
4. Затем `predictor/PROMPT.md` — 7 поведенческих паттернов пользователя (применять в ответах)
5. Если запрос пользователя касается 75% archive — спросить «зачем сейчас?» прежде чем работать
6. Если запрос касается 25% активного — работать без предисловий
7. Любое НОВОЕ предложение оценивать через призму P1-P4: усиливает ли путь экспоненты?
8. Если новое предложение не питает ни один из 4 путей — предложить отложить или применить к AILib/FMone
